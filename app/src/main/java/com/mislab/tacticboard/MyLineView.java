...
public class MyLineView extends View {
	...
	private void init(AttributeSet attrs, int defStyle) {
		// Load attributes
		/*final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.MyLineView, defStyle, 0);

		mExampleString = a.getString(R.styleable.MyLineView_exampleString);
		mExampleColor = a.getColor(R.styleable.MyLineView_exampleColor, mExampleColor);
		// Use getDimensionPixelSize or getDimensionPixelOffset when dealing
		// with
		// values that should fall on pixel boundaries.
		mExampleDimension = a.getDimension(R.styleable.MyLineView_exampleDimension, mExampleDimension);

		if (a.hasValue(R.styleable.MyLineView_exampleDrawable)) {
			mExampleDrawable = a.getDrawable(R.styleable.MyLineView_exampleDrawable);
...
*/
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawLine(paddingLeft+20, paddingTop + (contentHeight + mTextHeight) / 2, getWidth()-paddingRight-20, paddingTop + (contentHeight + mTextHeight) / 2, mTextPaint);
